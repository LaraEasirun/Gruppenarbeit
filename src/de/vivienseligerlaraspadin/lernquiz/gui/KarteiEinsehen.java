...
public KarteiEinsehen() {
	...
	String[] columnNames = {"ID", "Frage", "Antwort", "Kategorie", "Schwierigkeit", "Kann ich?" };
	...
	for (int i = 0; i < karteikartenListe.size(); i++) {
		...
		data[i][4] = karte.getSchwierigkeitsgrad();
		data[i][5] = karte.getBeherrscht() ? "Ja" : "Nein";
	}
...
private void aktualisiereTabelle() {
	...
	for (Karteikarte karte : karteikartenListe) {
		model.addRow(new Object[]{karte.getFrage(), karte.getAntwort(), karte.getKategorie(), karte.getSchwierigkeitsgrad()});
	}
}
